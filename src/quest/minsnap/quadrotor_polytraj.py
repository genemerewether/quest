...
from quest import exceptions
from quest import utils
from quest.minsnap import joint_optimize
...
class QRPolyTraj(object):
    ...
    def __init__(self, partial_waypoints, time_penalty, costs=None, der_fixed=None, der_ineq=None,
        ...
        Raises:
            quest.exceptions.InputError if the input values are of incorrect
                sizes or shapes
        """
        self.waypoints = dict()
    ...
    def relative_time_opt(self, **kwargs):
        ...
        cons = ({'type': 'ineq',
                 'fun': lambda x: (np.array(x) -
                                   np.ones((np.size(x))) * quest.settings.TIME_NZ_EPS)})
